...
def generate_image_from_image(image, prompt):
    image = generator_refiner(
        prompt=prompt,
        num_inference_steps=60,
        image=image,
...
def align_and_crop(image):
    ...
    if len(landmarks) > 0:
        ...
        print("#. Warning: No landmarks found in image")
...
for image_path in image_paths:
    ...
    prompt = caption_image(image) + " color face close-up photography"
    print(f"IMAGE: {image_path.stem}")
    print(f"PROMPT: {prompt}")
    synthetic_image = generate_image(prompt)
    while True:
        try:
            synthetic_image = align_and_crop(synthetic_image)
            break
        except ValueError:
            synthetic_image = generate_image(prompt)
    # synthetic_image = generate_image_from_image(
    #     synthetic_image.resize((1024, 1024)), prompt
    # )
    synthetic_image_exif = synthetic_image.getexif()
    synthetic_image_exif[0x9286] = prompt
    synthetic_image.save(
        args.output_dir / f"{image_path.stem}_synthetic.jpg",
        format="JPEG",
        quality=75,
        exif=synthetic_image_exif,
    )
    image.save(
